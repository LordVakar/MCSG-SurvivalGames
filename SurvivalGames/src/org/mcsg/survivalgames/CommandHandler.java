...
import org.mcsg.survivalgames.commands.DelSign;
import org.mcsg.survivalgames.commands.DelWall;
...
public class CommandHandler implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd1, String commandLabel, String[] args) {
		...
		if (SurvivalGames.config_todate == false) {
			msgmgr.sendMessage(PrefixType.WARNING, "The config file is out of date. Please tell an administrator to reset the config.",
					player);
			return true;
		}
			...
			if (args[0].equalsIgnoreCase("help")) {
				if (args.length == 1) {
					...
					if (args[1].toLowerCase().startsWith("admin")) {
						...
						msgmgr.sendMessage(PrefixType.WARNING, args[1] + " is not a valid page! Valid pages are Player, Staff, and Admin.",
								player);
					}
				}
				return true;
