...
public class LobbyManagerOld implements Listener {
	...
	public void loadSigns() {
		...
		byte temp = ((Sign) new Location(p.getServer().getWorld(c.getString("sg-system.lobby.sign.world")), x1, y1, z1).getBlock()
		...
		for (int y = vdiff - 1; y >= 0; y--) {
			for (int x = hdiff - 1; x >= 0; x--) {
				...
				BlockState b = p.getServer()
						.getWorld(SettingsManager.getInstance().getSystemConfig().getString("sg-system.lobby.sign.world")).getBlockAt(l)
						.getState();
	...
	public void setLobbySignsFromSelection(Player pl) {
		...
		if ((sel.getNativeMaximumPoint().getBlockX() - sel.getNativeMaximumPoint().getBlockX()) != 0
				&& (sel.getNativeMaximumPoint().getBlockZ() - sel.getNativeMaximumPoint().getBlockZ() != 0)) {
	...
	public void updateGameStatus() {
		...
		for (int a = 0; a < games.size(); a++) {
			try {
				...
				signs[b][1].setLine(2, game.getActivePlayers() + "/" + ChatColor.GRAY + game.getInactivePlayers() + ChatColor.BLACK + "/"
				...
				for (Player p : active) {
					if (signno < signs[b].length) {
						...
						signs[b][signno].setLine(line, (SurvivalGames.auth.contains(p.getName()) ? ChatColor.DARK_BLUE : ChatColor.BLACK)
								+ ((p.getName().equalsIgnoreCase("Double0negative")) ? "Double0" : p.getName()));
				...
				for (Player p : inactive) {
					if (signno < signs[b].length) {
						signs[b][signno].setLine(line, (SurvivalGames.auth.contains(p.getName()) ? ChatColor.DARK_RED : ChatColor.GRAY)
								+ ((p.getName().equalsIgnoreCase("Double0negative")) ? "Double0" : p.getName()));
						signs[b][signno].update();
						line++;
						if (line == 4) {
