...
public class SurvivalGames extends JavaPlugin {
	...
	public static List<String> auth = Arrays.asList(new String[] { "Double0negative", "iMalo", "Medic0987", "alex_markey", "skitscape",
			"AntVenom", "YoshiGenius", "pimpinpsp", "WinryR", "Jazed2011", "KiwiPantz", "blackracoon", "CuppingCakes", "4rr0ws", "Fawdz",
			"Timothy13", "rich91", "ModernPrestige", "Snowpool", "egoshk", "nickm140", "chaseoes", "Oceangrass", "GrailMore", "iAngelic",
			"Lexonia", "ChaskyT", "Anon232", "DragonCz", "TeeePeee" });
		...
		try {
			while (rs.next()) {
				if (stats.containsKey(rs.getString("player"))) {
					stats.put(
							rs.getString("player"),
							new Integer[] { stats.get(rs.getString("player"))[0] + rs.getInt("kills"),
	...
	public class HofUpdater implements Runnable {
		...
		public void run() {
			try {
				PreparedStatement st = DatabaseManager.getInstance().createStatement(
						"SELECT player, kills, death, points FROM " + SettingsManager.getSqlPrefix() + "playerstats;");
