...
public class AddSign implements SubCommand {
	...
	public boolean onCommand(Player player, String[] args) {
		...
		if (args.length != 1) {
			MessageManager.getInstance().sendFMessage(MessageManager.PrefixType.ERROR, "error.notspecified", player,
					"input-" + SettingsManager.getInstance().getMessageConfig().getString("messages.words.position", "Position"));
			return true;
		}
		int pos = 0;
		try {
	...
	public String help(Player p) {
		return "/sg addsign <pos> - "
				+ SettingsManager.getInstance().getMessageConfig()
						.getString("messages.help.addsign", "The sign you are looking at will show stats of player on <pos> position");
	}
