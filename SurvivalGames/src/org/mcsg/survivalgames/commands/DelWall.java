...
public class DelWall implements SubCommand {
	...
	public boolean onCommand(Player player, String[] args) {
		...
		if (!scfg.getBoolean("sg-system.lobby.sign.set", false)) { return true; }
		...
		for (int i = 0; i < scfg.getInt("sg-system.lobby.signno", 0) + 1; i++) {
			...
			if (scfg.getInt(path + "x1", 0) == max.getBlockX() && scfg.getInt(path + "y1", 0) == max.getBlockY()
					&& scfg.getInt(path + "z1", 0) == max.getBlockZ() && scfg.getInt(path + "x2", 0) == min.getBlockX()
					&& scfg.getInt(path + "y2", 0) == min.getBlockY() && scfg.getInt(path + "z2", 0) == min.getBlockZ()) {
	...
	public String help(Player p) {
		return "/sg addwall <id> - "
				+ SettingsManager.getInstance().getMessageConfig()
						.getString("messages.help.delwall", "Add a lobby stats wall for Arena <id>");
	}
