...
public class Disable implements SubCommand {
	...
	public boolean onCommand(Player player, String[] args) {
		...
		try {
			if (args.length == 0) {
				...
				MessageManager.getInstance().sendFMessage(MessageManager.PrefixType.INFO, "game.state", player, "arena-" + args[0],
						"input-disabled");
			}
	...
	public String help(Player p) {
		return "/sg disable <id> - "
				+ SettingsManager.getInstance().getMessageConfig().getString("messages.help.disable", "Disables arena <id>");
	}
