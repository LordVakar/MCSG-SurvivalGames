	...
	public void onPlayerClickEvent(PlayerInteractEvent event) {
		...
		try {
			if (GameManager.getInstance().isSpectator(player) && player.isSneaking()
					&& (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_AIR)
					|| GameManager.getInstance().isSpectator(player) && player.isSneaking()
				...
				if (i < 0) {
					i = (players.length - 1 < 0 ? players.length : players.length - 1);
				}
